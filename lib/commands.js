...
commands.remove = function(name) {
    if (commands._lookup[name]) {
        commands._lookup = commands._lookup.filter(function(cmd) {
            return cmd.name !== name;
        })
    }

    for (var i = 0; i < commands._cmds.length; i++) {
        var cmd = commands._cmds[i];
        if (cmd.name == name) {
            commands._cmds.splice(i, 1);
            i--;
        }
    }
}
